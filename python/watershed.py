...
if __name__ == "__main__":
    ...
    dist = cv2.distanceTransform(
        mask, cv2.DIST_L2, cv2.DIST_MASK_5, dstType=cv2.CV_32F)
    # locate center of each kernel
    thresh, dist_thresh = cv2.threshold(
        dist, 0.42 * np.max(dist), 255, cv2.THRESH_BINARY)
    ...
    dist_markers_colored = cv2.addWeighted(
        gray_img, 0.4, dist_markers_colored, 0.6, 0)
    morph_markers_colored = cv2.addWeighted(
        gray_img, 0.4, morph_markers_colored, 0.6, 0)
