...
src_img = cv2.imread('./images/boat.png')
# psf_img = cv2.imread('./images/blur_impulse.png')

f = cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
...
axs[1, 0].set_title('Richardson-Lucy, iterations=10')

axs[1, 1].imshow(f_r_50, cmap='gray', vmin=0, vmax=1)
axs[1, 1].set_title('Richardson-Lucy, iterations=50')

axs[1, 2].imshow(f_r_100, cmap='gray', vmin=0, vmax=1)
axs[1, 2].set_title('Richardson-Lucy, iterations=100')
